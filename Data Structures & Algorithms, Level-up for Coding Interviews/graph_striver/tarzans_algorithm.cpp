...
void dfs(int node,vvi arr,vi &vis,vi &onStack,vi &in,vi &low,stack<int> &st,int &timer,int &scc){
    vis[node] = 1;
    ...
    for(int u:arr[node]){
        if(vis[u] and onStack[u]){
            low[node] = min(low[node],in[u]);
        }else if(!vis[u]){
            dfs(u,arr,vis,onStack,in,low,st,timer,scc);
            if(onStack[u]){
